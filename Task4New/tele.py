...
# Load environment variables from vs.env
load_dotenv('vs.env')

# API code and Telegram token from environment variables
...
async def handle_start(update, context):
    await context.bot.send_message(chat_id=update.effective_chat.id, text="Welcome to BookBot! Use /help for commands.")

async def handle_book(update, context):
    await context.bot.send_message(chat_id=update.effective_chat.id, text="Please provide a book title or author to search for.")

async def handle_preview(update, context):
    await context.bot.send_message(chat_id=update.effective_chat.id, text="Please provide a book title or author to preview.")

async def handle_list(update, context):
    await context.bot.send_message(chat_id=update.effective_chat.id, text="Here's a list of popular books.")

async def handle_reading_list(update, context):
    await context.bot.send_message(chat_id=update.effective_chat.id, text="Here's your reading list.")
...
async def handle_genre_query(update, context):
    ...
    if books:
        for book in books[:5]:  # Show 5 books
            await context.bot.send_message(chat_id=update.effective_chat.id, text=format_book_details(book))
    else:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="No books found in this genre.")
